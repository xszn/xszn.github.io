...
import 'kleur/colors';
import { p as plainify, a as $$Image, s as slugify, h as humanize, c as config } from './_astro_content_C56GEhFt.mjs';
import { g as getSinglePage } from './contentParser_BHR6hCNO.mjs';
...
const $$Posts = createComponent(async ($$result, $$props, $$slots) => {
  ...
  return renderTemplate`${maybeRenderHead()}<div${addAttribute(`row gy-5 gx-4 ${className} ${posts.length == 1 ? "justify-center" : ""} `, "class")}> ${posts.map((post, i) => renderTemplate`<div${addAttribute(
    i === 0 && fluid != false ? "col-12" : "col-12 sm:col-6 lg:col-6 xl:col-6",
    "class"
  )}> ${post.data.image && renderTemplate`<a${addAttribute(`/${post.id}`, "href")} class="rounded-lg block hover:text-primary overflow-hidden group"> ${renderComponent($$result, "Image", $$Image, { "class": "group-hover:scale-[1.03] transition duration-300 w-full", "src": post.data.image, "alt": post.data.title, "width": i === 0 ? 925 : 445, "height": i === 0 ? 475 : 230 })} </a>`} <ul class="mt-6 mb-4 flex flex-wrap items-center text-text"> <li class="mr-5"> ${authors.filter(
